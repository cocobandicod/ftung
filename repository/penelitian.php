<?php
function penelitian($con,$link){
?>
<h3>Tabel Capaian Penelitian Dosen FT UNG</h3>
...
<tr align="center">
  ...
  <td bgcolor="<?= $j[warna]; ?>">
  <?php 
  $m = mysqli_num_rows(mysqli_query($con,"SELECT * FROM penelitian p, prodi d 
  ...
  <td bgcolor="<?= $j[warna]; ?>">
  <?php 
  $pt = mysqli_num_rows(mysqli_query($con,"SELECT * FROM penelitian p, prodi d 
  									 WHERE p.id_prodi = d.id_prodi
									 AND d.id_jurusan = '$j[id_jurusan]'
									 AND tahun = '$i'
									 AND sumber_dana = 'PT'"));
  ...
  <td bgcolor="<?= $j[warna]; ?>">
  <?php 
  $dks = mysqli_num_rows(mysqli_query($con,"SELECT * FROM penelitian p, prodi d 
  									 WHERE p.id_prodi = d.id_prodi
									 AND d.id_jurusan = '$j[id_jurusan]'
									 AND tahun = '$i'
									 AND sumber_dana = 'DKS'"));
  ...
  <td bgcolor="<?= $j[warna]; ?>">
  <?php 
  $idn = mysqli_num_rows(mysqli_query($con,"SELECT * FROM penelitian p, prodi d 
  									 WHERE p.id_prodi = d.id_prodi
									 AND d.id_jurusan = '$j[id_jurusan]'
									 AND tahun = '$i'
									 AND sumber_dana = 'IDN'"));
  ...
  <td bgcolor="<?= $j[warna]; ?>">
  <?php 
  $iln = mysqli_num_rows(mysqli_query($con,"SELECT * FROM penelitian p, prodi d 
  									 WHERE p.id_prodi = d.id_prodi
									 AND d.id_jurusan = '$j[id_jurusan]'
									 AND tahun = '$i'
									 AND sumber_dana = 'ILN'"));
...
?>
<h3>Tabel Penelitian Dosen <?= $p[nama_prodi]; ?></h3>
<div class="table-responsive">
<table class="table table-striped table-bordered add-manage-table table demo" data-filter="#filter" data-filter-text-only="true">
<tr align="center">
...
if($_GET[prodi] == 'fakultas-teknik'){
	$sqld = mysqli_query($con,"SELECT * FROM penelitian ORDER BY tahun ASC");
} else {
	$sqld = mysqli_query($con,"SELECT * FROM penelitian WHERE id_prodi = '$p[id_prodi]' ORDER BY tahun ASC");
}
