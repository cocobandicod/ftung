<?php
function del_tracer_study($con,$kode){
	$d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM tracer_study WHERE kode = '$kode'"));
	unlink("../berkas/$d[file_pendukung]");
	mysqli_query($con,"DELETE FROM tracer_study WHERE kode = '$kode'");	
	mysqli_query($con,"DELETE FROM berkas WHERE kode = '$kode'");	
	echo "<script>
    window.location=('tracer-study.html');</script>";
}
function input_tracer_study($con,$kode){
	require_once('../config/fungsi_upload_berkas.php');
	$s = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM tracer_study WHERE kode = '$kode'"));	
	$p = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM prodi WHERE id_prodi = '$s[id_prodi]'"));	
	$m = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM menu WHERE seo = 'tracer-study'"));	
?>
                    <?php
					if($_REQUEST[save]){
                                        $kode = rand(10000,100000);
										$enkript = base64_encode($con,$kode);
										$import="INSERT INTO tracer_study VALUES('$id',
                                                                               	   '$enkript',
																				   '$_POST[prodi]',
																				   '$_POST[judul]',
																				   '$_POST[tahun]',
									...
									echo "<script>
                                    window.location=('tracer-study-edit-$enkript.html');</script>";
					}
					if($_REQUEST[update]){
						
						mysqli_query ($con,"UPDATE tracer_study SET id_prodi 	 	= '$_POST[prodi]',
																judul 				= '$_POST[judul]',
																tahun				= '$_POST[tahun]'
																WHERE kode = '$kode'");		
						...
						mysqli_query ($con,"UPDATE berkas SET  judul 	= '$_POST[judul]'
									...
									echo "<script>
                                    window.location=('tracer-study-edit-$kode.html');</script>";
					}
					if($_GET[act] == 'del_file'){
					    $d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM tracer_study WHERE kode = '$kode'"));
						unlink("../../berkas/$d[file_pendukung]");
					    mysqli_query ($con,"UPDATE tracer_study SET file_pendukung = '' WHERE kode = '$kode'");
						mysqli_query ($con,"UPDATE berkas SET file_pendukung  = '',
														ket				= ''
														WHERE kode = '$kode'");
						echo "<script>
						window.location=('tracer-study-edit-$_GET[kode].html');</script>";
					}
					?>
...
<div class="col-lg-6">                    
                    ...
                    <tr>
                      ...
                      <td>
                           <?php 
							   ...
							   ?>
                                <a href="delete-file-tracer-study-<?= $_GET[kode]; ?>.html" 
                    ...
                    <div class="col-lg-12">
                      ...
                      <a href="tracer-study.html">Kembali</a>
                      <?php } else { ?>
                      <input type="submit" class="btn btn-primary" value="Update" name="update" /> |
                      <a href="tracer-study.html">Kembali</a> | <a href="tracer-study-add.html">Tambah Ulang</a>
                      <?php } ?><br /><br />                 
                    </div>
                    </form>
                    <?php
					if (isset($_REQUEST['upload_1'])) {
                       echo upload_file_FILE($_POST[nama_prodi],'Tracer_Study','tracer-study','tracer_study');
                    }
					?>
<?php } ?>