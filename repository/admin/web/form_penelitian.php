<?php
function del_penelitian($con,$kode){
	$d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM penelitian WHERE kode = '$kode'"));
	unlink("../berkas/$d[file_pendukung]");
	mysqli_query($con,"DELETE FROM penelitian WHERE kode = '$kode'");
	mysqli_query($con,"DELETE FROM berkas WHERE kode = '$kode'");	
	echo "<script>
    window.location=('penelitian.html');</script>";
}
function input_penelitian($con,$kode){
	require_once('../config/fungsi_upload_berkas.php');
	$s = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM penelitian WHERE kode = '$kode'"));	
	$p = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM prodi WHERE id_prodi = '$s[id_prodi]'"));
	$m = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM menu WHERE seo = 'penelitian'"));	
?>
                    <?php
					if($_REQUEST[save]){
                                        $kode = rand(10000,100000);
										$enkript = base64_encode($con,$kode);
										$import="INSERT INTO penelitian VALUES('$id',
                                                                               	   '$enkript',
																				   '$_POST[prodi]',
									...
									echo "<script>
                                    window.location=('penelitian-edit-$enkript.html');</script>";
					}
					if($_REQUEST[update]){

						mysqli_query ($con,"UPDATE penelitian SET id_prodi 	= '$_POST[prodi]',
																judul_penelitian = '$_POST[judul_penelitian]',
																sumber_dana 	 = '$_POST[sumber_dana]',
																tahun			 = '$_POST[tahun]'
																WHERE kode 		 = '$kode'");		

						mysqli_query ($con,"UPDATE penelitian SET berkas 	judul 	= '$_POST[judul_penelitian]',
																isi 	= '$_POST[sumber_dana]'
																WHERE kode 	= '$kode'");
                                    
									echo "<script>
                                    window.location=('penelitian-edit-$kode.html');</script>";
					}
					if($_GET[act] == 'del_file'){
					    $d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM penelitian WHERE kode = '$kode'"));
						unlink("../../berkas/$d[file_pendukung]");
					    mysqli_query ($con,"UPDATE penelitian SET file_pendukung = '' WHERE kode = '$kode'");
...
<div class="col-lg-6">                    
                    ...
                    <tr>
                      ...
                      <td>
                           <?php 
							   ...
							   ?>
                                <a href="delete-file-penelitian-<?= $_GET[kode]; ?>.html" 
                    ...
                    <div class="col-lg-12">
                      ...
                      <a href="penelitian.html">Kembali</a>
                      <?php } else { ?>
					...
					if (isset($_REQUEST['upload_1'])) {
                       echo upload_file_penelitian($_POST[nama_prodi]);
                    }
					?>
<?php } ?>