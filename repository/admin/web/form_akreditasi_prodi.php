...
function input_akreditasi_prodi($con,$kode){
	...
	$m = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM menu WHERE seo = 'akreditasi-prodi'"));	
?>
                    <?php
					if($_REQUEST[save]){
                                        $kode = rand(10000,100000);
										$enkript = base64_encode($con,$kode);
										$import="INSERT INTO akreditasi_prodi VALUES('$id',
                                                                               	   '$enkript',
																				   '$_POST[prodi]',
																				   '$_POST[judul]',
																				   '$_POST[tahun]',
					...
					if($_REQUEST[update]){
						...
						mysqli_query ($con,"UPDATE akreditasi_prodi SET id_prodi 	 	= '$_POST[prodi]',
																judul 				= '$_POST[judul]',
																tahun				= '$_POST[tahun]'
																WHERE kode = '$kode'");		
					...
					if($_GET[act] == 'del_file'){
					    $d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM akreditasi_prodi WHERE kode = '$kode'"));
						unlink("../../berkas/$d[file_pendukung]");
					    mysqli_query ($con,"UPDATE akreditasi_prodi SET file_pendukung = '' WHERE kode = '$kode'");
...
<div class="col-lg-6">                    
                    ...
                    <tr>
                      ...
                      <td>
                           <?php 
							   ...
							   ?>
                                <a href="delete-file-akreditasi-prodi-<?= $_GET[kode]; ?>.html" 
                    ...
                    <div class="col-lg-12">
                      ...
                      <a href="akreditasi-prodi.html">Kembali</a> | <a href="akreditasi-prodi-add.html">Tambah Ulang</a>
                      <?php } ?><br /><br />                 
                    </div>
                    </form>
                    <?php
					if (isset($_REQUEST['upload_1'])) {
                       echo upload_file_FILE($_POST[nama_prodi],'Akreditasi_Prodi','akreditasi-prodi','akreditasi_prodi');
                    }
					?>
<?php } ?>