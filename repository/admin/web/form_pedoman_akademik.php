<?php
function del_pedoman_akademik($con,$kode){
	$d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM pedoman_akademik WHERE kode = '$kode'"));
	unlink("../berkas/$d[file_pendukung]");
	mysqli_query($con,"DELETE FROM pedoman_akademik WHERE kode = '$kode'");	
	mysqli_query($con,"DELETE FROM berkas WHERE kode = '$kode'");	
	echo "<script>
    window.location=('pedoman-akademik.html');</script>";
}
function input_pedoman_akademik($con,$kode){
	require_once('../config/fungsi_upload_berkas.php');
	$s = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM pedoman_akademik WHERE kode = '$kode'"));	
	$p = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM prodi WHERE id_prodi = '$s[id_prodi]'"));	
	$m = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM menu WHERE seo = 'pedoman-akademik'"));	
?>
                    <?php
					if($_REQUEST[save]){
                                        $kode = rand(10000,100000);
										$enkript = base64_encode($con,$kode);
										$import="INSERT INTO pedoman_akademik VALUES('$id',
                                                                               	   '$enkript',
																				   '$_POST[prodi]',
																				   '$_POST[judul]',
																				   '$_POST[tahun]',
									...
									echo "<script>
                                    window.location=('pedoman-akademik-edit-$enkript.html');</script>";
					}
					if($_REQUEST[update]){
						
						mysqli_query ($con,"UPDATE pedoman_akademik SET id_prodi 	 	= '$_POST[prodi]',
																judul 				= '$_POST[judul]',
																tahun				= '$_POST[tahun]'
																WHERE kode = '$kode'");		
						...
						mysqli_query ($con,"UPDATE berkas SET  judul 	= '$_POST[judul]'
									...
									echo "<script>
                                    window.location=('pedoman-akademik-edit-$kode.html');</script>";
					}
					if($_GET[act] == 'del_file'){
					    $d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM pedoman_akademik WHERE kode = '$kode'"));
						unlink("../../berkas/$d[file_pendukung]");
					    mysqli_query ($con,"UPDATE pedoman_akademik SET file_pendukung = '' WHERE kode = '$kode'");
						mysqli_query ($con,"UPDATE berkas SET file_pendukung  = '',
														ket				= ''
														WHERE kode = '$kode'");
						echo "<script>
						window.location=('pedoman-akademik-edit-$_GET[kode].html');</script>";
					}
					?>
...
<div class="col-lg-6">                    
                    ...
                    <tr>
                      ...
                      <td>
                           <?php 
							   ...
							   ?>
                                <a href="delete-file-pedoman-akademik-<?= $_GET[kode]; ?>.html" 
                    ...
                    <div class="col-lg-12">
                      ...
                      <a href="pedoman-akademik.html">Kembali</a>
                      <?php } else { ?>
                      <input type="submit" class="btn btn-primary" value="Update" name="update" /> |
                      <a href="pedoman-akademik.html">Kembali</a> | <a href="pedoman-akademik-add.html">Tambah Ulang</a>
                      <?php } ?><br /><br />                 
                    </div>
                    </form>
                    <?php
					if (isset($_REQUEST['upload_1'])) {
                       echo upload_file_FILE($_POST[nama_prodi],'Pedoman_Akademik','pedoman-akademik','pedoman_akademik');
                    }
					?>
<?php } ?>