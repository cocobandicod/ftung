<?php
function del_rencana_operasional($con,$kode){
	$d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM rencana_operasional WHERE kode = '$kode'"));
	unlink("../berkas/$d[file_pendukung]");
	mysqli_query($con,"DELETE FROM rencana_operasional WHERE kode = '$kode'");	
	mysqli_query($con,"DELETE FROM berkas WHERE kode = '$kode'");	
	echo "<script>
    window.location=('rencana-operasional.html');</script>";
}
function input_rencana_operasional($con,$kode){
	require_once('../config/fungsi_upload_berkas.php');
	$s = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM rencana_operasional WHERE kode = '$kode'"));	
	$p = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM prodi WHERE id_prodi = '$s[id_prodi]'"));	
	$m = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM menu WHERE seo = 'rencana-operasional'"));	
?>
                    <?php
					if($_REQUEST[save]){
                                        $kode = rand(10000,100000);
										$enkript = base64_encode($con,$kode);
										$import="INSERT INTO rencana_operasional VALUES('$id',
                                                                               	   '$enkript',
																				   '$_POST[prodi]',
																				   '$_POST[judul]',
																				   '$_POST[tahun]',
									...
									echo "<script>
                                    window.location=('rencana-operasional-edit-$enkript.html');</script>";
					}
					if($_REQUEST[update]){
						
						mysqli_query ($con,"UPDATE rencana_operasional SET id_prodi 	 	= '$_POST[prodi]',
																judul 				= '$_POST[judul]',
																tahun				= '$_POST[tahun]'
																WHERE kode = '$kode'");		
						...
						mysqli_query ($con,"UPDATE berkas SET  judul 	= '$_POST[judul]'
									...
									echo "<script>
                                    window.location=('rencana-operasional-edit-$kode.html');</script>";
					}
					if($_GET[act] == 'del_file'){
					    $d = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM rencana_operasional WHERE kode = '$kode'"));
						unlink("../../berkas/$d[file_pendukung]");
					    mysqli_query ($con,"UPDATE rencana_operasional SET file_pendukung = '' WHERE kode = '$kode'");
						mysqli_query ($con,"UPDATE berkas SET file_pendukung  = '',
														ket				= ''
														WHERE kode = '$kode'");
						echo "<script>
						window.location=('rencana-operasional-edit-$_GET[kode].html');</script>";
					}
					?>
...
<div class="col-lg-6">                    
                    ...
                    <tr>
                      ...
                      <td>
                           <?php 
							   ...
							   ?>
                                <a href="delete-file-rencana-operasional-<?= $_GET[kode]; ?>.html" 
                    ...
                    <div class="col-lg-12">
                      ...
                      <a href="rencana-operasional.html">Kembali</a>
                      <?php } else { ?>
                      <input type="submit" class="btn btn-primary" value="Update" name="update" /> |
                      <a href="rencana-operasional.html">Kembali</a> | <a href="rencana-operasional-add.html">Tambah Ulang</a>
                      <?php } ?><br /><br />                 
                    </div>
                    </form>
                    <?php
					if (isset($_REQUEST['upload_1'])) {
                       echo upload_file_FILE($_POST[nama_prodi],'Rencana_Operasional','rencana-operasional','rencana_operasional');
                    }
					?>
<?php } ?>