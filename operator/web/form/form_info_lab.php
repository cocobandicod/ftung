...
cek_url($url2, $proses, $_GET['act'], 'jurusan', 'id_jurusan = "' . @$_SESSION['id_jurusan'] . '"');
$p = $proses->tampil_data_saja('*', 'jurusan', '1=1 AND id_jurusan = "' . @$_SESSION['id_jurusan'] . '"');
?>
<!DOCTYPE html>
                    ...
                    <div class="page-header-content d-lg-flex">
                        <div class="d-flex">
                            <h4 class="page-title mb-0">
                                Laboratorium Info <?= ucfirst($_GET['act']) . ' ' . str_replace('-', ' ', $p['nama_jurusan']); ?>
                            </h4>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <form method="post" id="form" class="form" enctype="multipart/form-data">
                                    <?php
                                    if ($_GET['act'] == 'edit') {
                                        $row = $proses->tampil_data_saja('*', 'profil_jurusan', 'id_profil = ' . $_GET['id'] . '');
...
<script>
    ...
    $(document).on('submit', '#form', function(event) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= $url2; ?>proses/profil/jurusan',
            ...
            success: function(data) {
                ...
                setTimeout(function() {
                    window.location = "<?= $url2 ?>laboratorium/info-laboratorium";
                }, 2000)
            },
