...
cek_url($url2, $proses, $_GET['act'], 'layanan', 'id = "' . @$_GET['id'] . '"');
?>
<!DOCTYPE html>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <form method="post" id="form" class="form" enctype="multipart/form-data">
                                    <?php
                                    if ($_GET['act'] == 'edit') {
                                        $row = $proses->tampil_data_saja('*', 'layanan', 'id = ' . $_GET['id'] . '');
...
<script>
    ...
    const image_upload_handler_callback = (blobInfo, progress) => new Promise((resolve, reject) => {
        ...
        xhr.open('POST', '<?= $url2; ?>proses/upload/layanan');
    ...
    tinymce.init({
        ...
        images_upload_url: '<?= $url2; ?>proses/upload/layanan',
        ...
        swalInit.fire({
            ...
            if (result.value) {
                $.ajax({
                    ...
                    success: function(data) {
                        setTimeout(function() {
                            window.location = "<?= $url2 ?>layanan/edit/<?= @$_GET['id']; ?>";
                        }, 2000)
                    }
                });
                swalInit.fire(
