...
cek_url($url2, $proses, $_GET['act'], 'fasilitas', 'id_fasilitas = "' . @$_GET['id'] . '"');
?>
<!DOCTYPE html>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <form method="post" id="form" class="form" enctype="multipart/form-data">
                                    <?php
                                    if ($_GET['act'] == 'edit') {
                                        $row = $proses->tampil_data_saja('*', 'fasilitas', 'id_fasilitas = ' . $_GET['id'] . '');
                                        echo '<input type="hidden" name="id" value="' . $row['id_fasilitas'] . '">';
                                        ...
                                        if (empty($row['gambar'])) {
                                            ...
                                            <img src="' . $url . 'assets/images/fasilitas/' . $row['gambar'] . '" class="img-thumbnail">
                                            <div class="form-text text-muted"><a href="#" id="del" data-id="' . $row['id_fasilitas'] . '" data-file="' . $row['gambar'] . '" data-nama="' . $row['keterangan'] . '" data-act="image"><i class="ph-trash me-1"></i>Hapus Gambar</a></div>
                                            </div>';
                                            $required = '';
                                            $ket = '<br>*Kosongkan jika tidak merubah gambar';
                                        }
        ...
        $.ajax({
            ...
            success: function(data) {
                new Noty({
                    ...
                    setTimeout(function() {
                        window.location = "<?= $url2 ?>fasilitas";
                    }, 2000)
                );
            },
