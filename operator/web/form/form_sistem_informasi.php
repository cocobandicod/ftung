...
cek_url($url2, $proses, $_GET['act'], 'sistem_informasi', 'id_link = "' . @$_GET['id'] . '"');
?>
<!DOCTYPE html>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <form method="post" id="form" class="form" enctype="multipart/form-data">
                                    <?php
                                    if ($_GET['act'] == 'edit') {
                                        $row = $proses->tampil_data_saja('*', 'sistem_informasi', 'id_link = ' . $_GET['id'] . '');
                                        ...
                                        if (empty($row['file'])) {
                                            ...
                                            <img src="' . $url . 'assets/img/sistem_informasi/' . $row['file'] . '" class="img-thumbnail">
                                            <div class="form-text text-muted"><a href="#" id="del" data-id="' . $row['id_link'] . '" data-file="' . $row['file'] . '" data-nama="' . $row['judul'] . '" data-act="image"><i class="ph-trash me-1"></i>Hapus Gambar</a></div>
                                            </div>';
                                            $required = '';
                                            $ket = '<br>*Kosongkan jika tidak merubah gambar';
                                        }
...
<script>
    ...
    $(document).on('submit', '#form', function(event) {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= $url2; ?>proses/sistem/informasi',
            ...
            success: function(data) {
                new Noty({
                    ...
                    setTimeout(function() {
                        window.location = "<?= $url2 ?>sistem/informasi";
                    }, 2000)
                );
            },
